					...
					if(d.errors[i].fatal || d.errors[i].connection) {
						...
						if(d.errors[i].connection && $('#vboxPane').data('vboxConfig')	) {
							...
							if($('#vboxPane').data('vboxConfig').servers && $('#vboxPane').data('vboxConfig').servers.length) {
								...
								s = '<div style="display: block">'+trans('Server List','Desktop Virtualization')+': '+servers.join(', ')+'</div>';
							}
							if(s) vboxAlert(s);
							vboxAlert(d.errors[i],{'width':'400px'});
							vboxAlert('<p>'+trans('An error occurred communicating with your vboxwebsrv. No more requests will be sent by Desktop Virtualization Project until the error is corrected and this page is refreshed. The details of this connection error should be displayed in a subsequent dialog box.','Desktop_virtualization')+'</p>'+s,{'width':'50%'});
...
function vboxAlert(e,xtraOpts) {
	...
	var dialogOpts = {'closeOnEscape':false,'width':600,'height':'auto','buttons':buttons,'modal':true,'autoOpen':true,'stack':true,'dialogClass':'vboxDialogContent','title':'<img src="images/vbox/OSE/about_16px.png" class="vboxDialogTitleIcon" /> Desktop_Virtualization'};
...
function vboxConfirm(q,buttons,cancelText) {
    ...
    $(div).dialog({'closeOnEscape':false,'width':500,'height':'auto','buttons':buttons,'modal':true,'autoOpen':true,'stack':true,'dialogClass':'vboxDialogContent','title':'<img src="images/vbox/OSE/about_16px.png" class="vboxDialogTitleIcon" /> Desktop_virtualization'});
...
function vboxProgressCreateDialog(prequest,icon,title,target,callback) {
	...
	var div = $('<div />').attr({'id':'vboxProgress'+pid,'title':(title ? title : 'Desktop_virtualization'),'style':'text-align: center'});
 ...
 */
function vboxOpInProgress() { return trans('Warning: A VirtualBox internal operation is in progress. Closing this window or navigating away from this web page may cause unexpected and undesirable results. Please wait for the operation to complete.','Desktop_virtualization');}
...
function vboxProgressUpdate(prequest,d,modal) {
	...
	if(!d || !d.responseData || !d.responseData['progress'] || !d.responseData['info'] || d.responseData['info']['completed'] || d.responseData['info']['canceled']) {
		...
		if(d && d.responseData['info'] && d.responseData['info']['canceled'])
			vboxAlert(trans('Operation Canceled','Desktop_virtualization'),{'width':'100px','height':'auto'});
 ...
 */
function Desktop_virtualizationFailure() {
	if($('#vboxPane').data('vboxFatalError')) return;
	$('#vboxPane').data('vboxFatalError', 1);
	$('#vboxPane').css({'display':'none'});
	$('#vboxPane').trigger('Desktop_virtualizationFailure');
	vboxAlert(trans('There was an error obtaining the list of registered virtual machines from VirtualBox. Make sure vboxwebsrv is running and that the settings in config.php are correct.<p>The list of virtual machines will not begin auto-refreshing again until this page is reloaded.</p>','Desktop_virtualization'));
}
